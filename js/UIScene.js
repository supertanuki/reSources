class UIScene extends Phaser.Scene {
  ...
  _buildHUD() {
    const W = GAME_WIDTH, H = UI_HEIGHT;
    ...
    // Wood counter
    this.woodText = this.add.text(945, H / 2, 'Wood: 0 / 5', {
      fontSize: '14px', fill: '#ddbb55', fontFamily: 'monospace'
    }).setOrigin(0, 0.5);
  ...
  _makeButton(x, y, w, label, cb) {
    ...
    const txt = this.add.text(x + w / 2, y + h / 2, label, {
      fontSize: '13px', fontStyle: 'bold', fill: '#ffffff', fontFamily: 'monospace'
    }).setOrigin(0.5);
  ...
  _buildAlertPopup() {
    ...
    this.alertPopup = this.add.container(px, py).setVisible(false).setDepth(100);
  ...
  _buildGameOverPopup() {
    ...
    this.gameOverPopup = this.add.container(px, py).setVisible(false).setDepth(100);
    ...
    this.gameOverLabel = this.add.text(W / 2, 70, '', {
      fontSize: '19px', fill: '#ff5555', fontFamily: 'monospace',
      align: 'center', wordWrap: { width: W - 40 }
    }).setOrigin(0.5);
    ...
    const replayZone = this.add.zone(W / 2 - 80, H - 60, 160, 38)
      .setOrigin(0).setInteractive({ useHandCursor: true });
    ...
    this.gameOverPopup.add([bg, this.gameOverLabel, replayBg, replayTxt, replayZone]);
  }
  ...
  update() {
    this._updateBars();
    this._refreshButtons();
    this.woodText.setText(`Wood: ${GameState.wood} / ${GameState.BUILDING_WOOD_COST}`);
      ...
      const color = value < 20
        ...
        : value < 40
          ? Phaser.Display.Color.GetColor(
              Math.round(Phaser.Math.Linear(255, (item.def.color >> 16) & 0xff, (value - 20) / 20)),
              Math.round(Phaser.Math.Linear(51,  (item.def.color >> 8)  & 0xff, (value - 20) / 20)),
              Math.round(Phaser.Math.Linear(51,   item.def.color        & 0xff, (value - 20) / 20))
            )
          : item.def.color;
      item.fill.fillStyle(color, 1);
      item.fill.fillRoundedRect(item.def.x, 26, Math.max(0, 160 * value / 100), 14, 3);
